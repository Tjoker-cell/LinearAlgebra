...
<div	>
	<div class="super-content">
		...
		<div  style="position: absolute;left:680px;top:230px;">
				<span style="font-size: 18px;">矩阵相加结果为：</span>
		...
		<div>
         ...
         <div>
			<span style="font-size: 18px;">矩阵相减结果为：</span>
		<table id="te" style=""></table>
		</div>
		</div>
		</div></div>
		</body>
...
function getMess(){
		...
		if(colsA!=colsB||rowsA!=rowsB){
			alert("此两矩阵不能相加减,请是矩阵A的行列等于矩阵B的行列");
			return;
		}
	...
	$.ajax({
		...
		type:"post",
		data:{data:JSON.stringify(Data),listA:listA,listB:listB,name:"jiajiang"},
		...
		success:function(data){
			...
			for(t;t<2;t++){
				var temp;
			for(var rows=0;rows<value[t].row;rows++){
				temp+='<tr>';
				for(var cols=0;cols<value[t].col;cols++){
					temp+='<td>'+'<input  style="max-width: 60px;" type="number"  value="'+value[t].data[rows][cols]+'"/>'+'</td>';
					
					alert(value[t].data[rows][cols]);
				}
				cols=0;
				temp+='</tr>';
